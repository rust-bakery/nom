...
| [take_while](https://docs.rs/nom/latest/bytes/complete/fn.take_while.html) | `take_while(is_alphabetic)` |  `"abc123"` | `Ok(("123", "abc"))` |Returns the longest list of bytes for which the provided function returns true. `take_while1` does the same, but must return at least one character|
...
| [fold_separated0](https://docs.rs/nom/latest/nom/multi/fn.fold_separated0.html) | `fold_separated0(tag(","), tag("ab"), || 0, |l, o: &str| l + o.len())` | `"ab,ab,ab."` | `Ok((".", 6))` |`fold_separated1` works like `fold_separated0` but must accumulate at least one element|
