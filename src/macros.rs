...
/// `chain_group!(group_name, I->IResult<I,A> ~ I->IResult<I,B> ~ ... I->IResult<I,X> , || { return O } ) => I -> IResult<I, O>`
/// Like `chain!`, chains parsers and assemble the results through a closure
/// `chain_group` makes the entire parsed chain available as a single field with the specified `group_name`
/// You can use this to emulate capture groups in regular expressions
/// The input type must implement nom::InputLength
/// This combinator will count how much data is consumed by every child parser and take it into account if
/// there is not enough data
///
/// ```
...
///
///  named!(z<&[u8], B>,
///    chain_group!(whole_str
///      tag!("abcd")  ~     // the '~' character is used as separator
///      aa: ret_int   ~     // the result of that parser will be used in the closure
///      tag!("abcd")? ~     // this parser is optional
///      bb: ret_y?    ,     // the result of that parser is an option
///                          // the last parser in the chain is followed by a ','
///      ||{B{a: aa, b: bb, w: whole_str}}
///    )
///  );
///
/// # fn main() {
...
/// // everything is present, everything is parsed
/// let r2 = z(&b"abcdabcdefgh"[..]);
/// assert_eq!(r2, Done(&b""[..], B{a: 1, b: Some(1), w: &b"abcdabcdefgh"[..]}));
///
/// // the second "abcd" tag is optional
/// let r3 = z(&b"abcdefgh"[..]);
/// assert_eq!(r3, Done(&b""[..], B{a: 1, b: Some(1), w: &b"abcdefgh"[..]}));
///
/// // the result of ret_y is optional, as seen in the B structure
/// let r4 = z(&b"abcdabcdwxyz"[..]);
/// assert_eq!(r4, Done(&b"wxyz"[..], B{a: 1, b: None, &b"abcdabcd"[..]}));
/// # }
/// ```
#[macro_export]
macro_rules! chain_group (
  ($i:expr, $group:ident, $($rest:tt)*) => (
    {
      chaining_group_parser!($i, 0usize, $i => $group# $($rest)*)
    }
  );
);
...
#[doc(hidden)]
#[macro_export]
macro_rules! chaining_group_parser (
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $e:ident ~ $($rest:tt)*) => (
    chaining_group_parser!($i, $consumed, $orig => $group# call!($e) ~ $($rest)*);
  );
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $submac:ident!( $($args:tt)* ) ~ $($rest:tt)*) => (
    {
      use $crate::InputLength;
    match $submac!($i, $($args)*) {
      $crate::IResult::Error(e)      => $crate::IResult::Error(e),
      ...
      $crate::IResult::Incomplete($crate::Needed::Size(i)) => $crate::IResult::Incomplete($crate::Needed::Size($consumed + i)),
      $crate::IResult::Done(i,_)     => {
        chaining_group_parser!(i, $consumed + (($i).input_len() - i.input_len()), $orig => $group# $($rest)*)
      }
    }
    }
);
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $submac:ident!( $($args:tt)* ) ? ~ $($rest:tt)*) => ({
    ...
    if let $crate::IResult::Incomplete(inc) = res {
      ...
      chaining_group_parser!(input, $consumed + (($i).input_len() - input.input_len()), $orig => $group# $($rest)*)
    }
    }
  });

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $orig:expr => $group:ident# $e:ident ~ $($rest:tt)*) => (
    chaining_group_parser!($i, $consumed, $orig => $group# $field: call!($e) ~ $($rest)*);
  );

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $submac:ident!( $($args:tt)* ) ~ $($rest:tt)*) => (
    {
      use $crate::InputLength;
    match  $submac!($i, $($args)*) {
      $crate::IResult::Error(e)      => $crate::IResult::Error(e),
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# mut $field:ident : $e:ident ~ $($rest:tt)*) => (
    chaining_group_parser!($i, $consumed, $orig => $group# mut $field: call!($e) ~ $($rest)*);
  );

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# mut $field:ident : $submac:ident!( $($args:tt)* ) ~ $($rest:tt)*) => (
    {
      use $crate::InputLength;
    match  $submac!($i, $($args)*) {
      $crate::IResult::Error(e)      => $crate::IResult::Error(e),
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $e:ident ? ~ $($rest:tt)*) => (
    chaining_group_parser!($i, $consumed, $orig => $group# $field : call!($e) ? ~ $($rest)*);
  );
  ...
  // ending the chain
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $e:ident, $assemble:expr) => (
    chaining_group_parser!($i, $consumed, $orig => $group# call!($e), $assemble);
  );

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $submac:ident!( $($args:tt)* ), $assemble:expr) => (
    ...
    match $submac!($i, $($args)*) {
      ...
      $crate::IResult::Incomplete($crate::Needed::Size(i)) => $crate::IResult::Incomplete($crate::Needed::Size($consumed + i)),
      $crate::IResult::Done(i,o)     => {
        let $group = &($orig)[..$consumed + ($i).input_len() - i.input_len()];
        $crate::IResult::Done(i, $assemble())
      }
    }
  );
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $submac:ident!( $($args:tt)* ) ?, $assemble:expr) => ({
    ...
    if let $crate::IResult::Incomplete(inc) = res {
      ...
      let (input, parsed_len) = if let $crate::IResult::Done(i,o) = res {
        (i, ($i).input_len() - i.input_len())
      } else {
        ($i, 0)
      };
      let $group = &($orig)[..$consumed + parsed_len];
      $crate::IResult::Done(input, $assemble())
    }
  });

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $e:ident, $assemble:expr) => (
    chaining_group_parser!($i, $consumed, $orig => $group# $field: call!($e), $assemble);
  );

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $submac:ident!( $($args:tt)* ), $assemble:expr) => (
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# mut $field:ident : $e:ident, $assemble:expr) => (
    chaining_group_parser!($i, $consumed, $orig => $group# mut $field: call!($e), $assemble);
  );

  ($i:expr, $consumed:expr, $orig:expr => $group:ident# mut $field:ident : $submac:ident!( $($args:tt)* ), $assemble:expr) => (
    match $submac!($i, $($args)*) {
      $crate::IResult::Error(e)      => $crate::IResult::Error(e),
      $crate::IResult::Incomplete($crate::Needed::Unknown) => $crate::IResult::Incomplete($crate::Needed::Unknown),
      $crate::IResult::Incomplete($crate::Needed::Size(i)) => $crate::IResult::Incomplete($crate::Needed::Size($consumed + i)),
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $field:ident : $submac:ident!( $($args:tt)* ) ? , $assemble:expr) => ({
    ...
    if let $crate::IResult::Incomplete(inc) = res {
      ...
      let ($field,input,parsed_len) = if let $crate::IResult::Done(i,o) = res {
        (::std::option::Option::Some(o), i, ($i).input_len() - i.input_len())
      } else {
        (::std::option::Option::None, $i, 0)
      };
      let $group = &($orig)[..$consumed + parsed_len];
      $crate::IResult::Done(input, $assemble())
    }
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# mut $field:ident : $submac:ident!( $($args:tt)* ) ? , $assemble:expr) => ({
    ...
    if let $crate::IResult::Incomplete(inc) = res {
      ...
      let (mut $field,input,parsed_len) = if let $crate::IResult::Done(i,o) = res {
        (::std::option::Option::Some(o), i, ($i).input_len() - i.input_len())
      } else {
        (::std::option::Option::None, $i, 0)
      };
  ...
  ($i:expr, $consumed:expr, $orig:expr => $group:ident# $assemble:expr) => (
    let $group = &($orig)[..$consumed]
    $crate::IResult::Done($i, $assemble())
  )
);
...
mod tests {
  ...
  enum TimeOffset<'a> {
    Z,
    Offset(&'a str, HourMinute<'a>, &'a str),
  }
  #[derive(PartialEq,Eq,Debug)]
  struct HourMinute<'a> {
    hour: &'a str,
  ...
  named!(date_time<&str, DateTime>,
       chain_group!(dt,
              date: chain_group!(date_str,
                           year:   is_a_s!("0123456789") ~
                                   tag_s!("/") ~
                            month: is_a_s!("0123456789") ~
                                   tag_s!("/") ~
                           day:    is_a_s!("0123456789") ,
                           ||{Date{year: year, month: day, day: month, full: date_str}}
                    ) ~  // chain_group!(date_str)
                    tag_s!(" ") ~
              time: chain_group!(time_str,
                          hour_minute: chain_group!(time_hr_min,
                                              time_hr:  is_a_s!("0123456789")~
                                                        tag_s!(":")          ~
                                              time_min: is_a_s!("0123456789"),
                                              ||{HourMinute{hour: time_hr, minute: time_min, full: time_hr_min}}
                                       ) ~  // chain_group!(time_hr_min)
                                       tag_s!(":") ~
                          second:      chain_group!(decimal,
                                              whole: is_a_s!("0123456789") ~
                                                     tag_s!(".") ~
                                              frac:  is_a_s!("0123456789") ,
                                              ||{Second{whole: whole, fractional: frac, full: decimal}}
                                       ) ~  // chain_group!(decimal)
                          offset:      alt!(complete!(tag_s!("Z")) => { |_| TimeOffset::Z } |
                                            complete!(
                                             chain_group!(off,
                                                    posneg: alt!(complete!(tag_s!("+")) | complete!(tag_s!("-"))) ~
                                                    hr_min: chain_group!(off_hr_min,
                                                                   off_hr:  is_a_s!("0123456789")~
                                                                            tag_s!(":")          ~
                          ...
                          ||{Time{hour_minute: hour_minute, second: second, offset: offset, full: time_str}}
                    ),  // chain_group!(time_str)
              ||{DateTime{date: date, time: time, full: dt}}
       )  // chain_group!(dt)
  );
  // The above named function is equivalent to this regular expression:
  // ((([0-9]*)\/([0-9]*)\/([0-9]*)) (([0-9]*):([0-9]*):([0-9]*)\.([0-9]*)(Z|(\+|-)([0-9]*):([0-9]*))))
  #[test]
  fn test_chain_group() {
    let datetime = date_time("2016/01/26 10:13:42.654+10:30");
    assert_eq!(datetime, Done("", DateTime {
                                   date: Date {
                                     year: "2016", month: "26", day: "01", full: "2016/01/26"
                                   },
                                   time: Time {
                                     hour_minute: HourMinute {
                                       hour: "10", minute: "13", full: "10:13"
                                     },
                                     second: Second {
                                     ...
                                     full: "10:13:42.654+10:30"
                                   },
                                   full: "2016/01/26 10:13:42.654+10:30"
                                 }));
  }
}
