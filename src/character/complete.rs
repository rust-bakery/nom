...
use crate::traits::{AsChar, FindToken, Input, InputLength, Slice};
...
macro_rules! ints {
    ($($t:tt)+) => {
        ...
        pub fn $t<T, E: ParseError<T>>(input: T) -> IResult<T, $t, E>
            where
            T: Input + Slice<RangeFrom<usize>> + Clone,
            <T as Input>::Item: AsChar,
            T: for <'a> Compare<&'a[u8]>,
            {
